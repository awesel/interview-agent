...
export default function DashboardDetailPage({ params }: { params: Promise<{ slug: string }> }){
  ...
  useEffect(()=>{ if(!slug) return; let cancelled=false; (async function load(){
    ...
    try {
      ...
      if(!cancelled){ setLoadError('Failed to load interview'); setRecordLoading(false); setSessionsLoading(false); }
    }
  ...
  return (
    <main style={{display:'flex', height:'calc(100dvh - 56px)'}}>
      ...
      <div style={{flex:1, minWidth:0, overflow:'auto', padding:'1.25rem 1.5rem'}}>
        {loadError && <div style={{background:'#fff8e1', border:'1px solid #ffe08a', color:'#7a5a00', padding:'8px 10px', borderRadius:8, fontSize:'0.6rem', marginBottom:10}}>{loadError}</div>}
